...
const AdminLogin = () => {
  ...
  useEffect(() => {
    ...
    // Check if the admin is already registered by making a GET request to the backend
    axios
      .get("http://localhost:3500/api/auth/checkAdmin")
      .then((response) => {
        if (response.data.success) {
          setIsRegistered(true); // Admin already exists, so allow login only
        } else {
          setIsRegistered(false); // If no admin is registered, allow registration
        }
      })
      .catch((error) => {
        console.log(error);
